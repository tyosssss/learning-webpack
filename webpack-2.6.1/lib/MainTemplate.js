...
module.exports = class MainTemplate extends Template {
  constructor(outputOptions) {
    ...
    this.plugin("startup", (source, chunk, hash) => {
      ...
      if (chunk.entryModule) {
        ...
        buf.push(`return ${this.renderRequireFunctionForModule(
          hash,
          chunk,
          JSON.stringify(chunk.entryModule.id))}(${this.requireFn}.s = ${JSON.stringify(chunk.entryModule.id)});`);
      }
      return this.asString(buf);
    });
   ...
   * @returns {ConcatSource} 返回渲染好的数据源
   */
  render(hash, chunk, moduleTemplate, dependencyTemplates) {
    ...
    let source = this.applyPluginsWaterfall("render",
   ...
   * @param {String} hash hash值
   * @param {Chunk} chunk 块
   * @param {String} varModuleId 模块ID的变量名
   * @param {String} varModule 模块的变量名
   * @returns {String}
   */
